...
import {ADMINTICKETLIST} from './admin-table';
import {DecimalPipe} from '@angular/common';
import {debounceTime, delay, switchMap, tap} from 'rxjs/operators';
import {SortDirection} from '../sortable.directive';

interface SearchResult {
        ticketlists: AdminTicket[];
        total: number;
      }
...
function sort(ticketlists: AdminTicket[], column: string, direction: string): AdminTicket[] {
  if (direction === '') {
    return ticketlists;
  } else {
...
function matches(ticket: AdminTicket, term: string, pipe: PipeTransform) {
  return ticket.CName.toLowerCase().includes(term.toLowerCase())
  || ticket.CRegNumber.toLowerCase().includes(term.toLowerCase())
...
export class AdminListService {
  ...
  private _ticketlists$ = new BehaviorSubject<AdminTicket[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let ticketlists = sort(ADMINTICKETLIST, sortColumn, sortDirection);
